...
class TestRunStore(unittest.TestCase):
    ...
    def test_get_detail_includes_default_state_and_trace(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = RunStore(Path(tmp))
            run = store.create(session_id="session_001", user_input="hello")

            detail = store.get_detail(run["id"])

            self.assertEqual(detail["id"], run["id"])
            self.assertEqual(detail["state"]["status"], "not_started")
            self.assertEqual(detail["trace"]["events"], [])
