...
DEFAULT_SUGGESTIONS = [
    ...
    CommandSuggestion("/attach <path>", "添加模板或参考附件", "/attach "),
    CommandSuggestion("/attach template <path>", "添加Word模板", "/attach template "),
    CommandSuggestion("/attach reference <path>", "添加参考资料", "/attach reference "),
    CommandSuggestion("/attachments", "查看当前会话附件"),
    CommandSuggestion("/detach <attachment-id>", "移除当前会话附件", "/detach "),
    CommandSuggestion("/document status", "查看当前文档任务"),
    CommandSuggestion("/document history", "查看文档版本历史"),
    CommandSuggestion("/document rollback <version>", "切换到历史文档版本", "/document rollback "),
