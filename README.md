...
## Document Format Normalizer Demo

YCore includes a `document-format-normalizer` skill for Word document format normalization. It accepts a source `.docx` draft, applies the built-in `report-standard` template by default, writes a normalized `.docx`, and emits a Markdown/JSON audit report.

Example user request:

    Use document-format-normalizer on draft.docx with the report-standard template. Save the result as normalized-report.

The generated files are written under the active workspace `.ycore/docx-format/` directory.
