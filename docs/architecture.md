...
YCore 是一个通用的 skill-driven 本地 Agent Harness。它面向中文用户，把 Skill 工作流、工具调用、workspace/context、memory、trace/state、eval 和 verification 收敛到一个可复盘的 CLI runtime 中。

具体业务能力由 Skill 决定。当前第一批验证 Skill 是 `code-review` 和 `eval-writer`；它们用于证明 Harness 能支撑本地项目审查和 eval 设计 workflow，但不代表 YCore 被固定成某个单一领域产品。
...
这一阶段不实现真实 stdio MCP client，因为当前验证 Skill 暂时不必须依赖 GitHub、浏览器、数据库或知识库 MCP。真正接入生产 MCP 时，需要补充 stdio 进程生命周期、initialize/tools/list 协议、JSON-RPC 消息关联、资源 metadata、认证、超时、取消、日志脱敏和错误恢复。
...
- 只保留 CLI 端。
- 当前只发布 `code-review` 与 `eval-writer` 两个示例业务 Skill。
